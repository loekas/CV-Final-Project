...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
    ...
    for k = 1:length(feat_comb)
        if k == length(feat_comb)
            [matches_buf,~] = vl_ubcmatch(cell2mat(descr_comb(1,k)), cell2mat(descr_comb(1,1)) ); % For the last frame compare the matches with the first frame
        else 
            [matches_buf,~] = vl_ubcmatch(cell2mat(descr_comb(1,k)), cell2mat(descr_comb(1,k +1)) ); % For the rest frames compare the current frame with the next frame
        end
