function Points = ExtractPixel4(Blocks, inliers_matches, inliers_matched_features_combined)
...
[~, I_match] = ismember(Blocks{18},inliers_matches{1,18}(1,:));
        ...
        points_y(1,:) = inliers_matched_features_combined{1,18}(1,I_match(1,:));
        
[~, I_match] = ismember(Blocks{18},inliers_matches{1,18}(2,:));
        points_x(2,:) = inliers_matched_features_combined{1,18}(2,I_match(2,:));
        points_y(2,:) = inliers_matched_features_combined{1,18}(2,I_match(2,:));

[~, I_match] = ismember(Blocks{18},inliers_matches{1,19}(2,:));
        points_x(3,:) = inliers_matched_features_combined{1,19}(2,I_match(3,:));
